...
sub usage {
    ...
    if ($arch eq "w32") {
        print "                 (c, cpp, java, cpp_borland, csharp, python, xsd)\n";
    }
    else { # assume x64
        print "                 (c_64, cpp_64, java, csharp, python, xsd)\n";
    }
...
my $known_subdir = 0;
if ($subdir eq "java" || $subdir eq "csharp" || $subdir eq "xsd") {
    $known_subdir = 1;
}
...
die ("$subdir is not a known subdirectory " .
     '(must be c|c_64, cpp|cpp_64, csharp, java, python, or xsd)')
    unless ($known_subdir);

die ("$rules are not valid encoding rules\n" . 
     "(must be ber, cer, der, json, mder, oer, per, xer, xml,\n" .
     " test_*, unit_*, or all)") 
...
if ($subdir eq "xsd") {
    ...
    &execSamples ("sample_uper", $function);
    print "--- Executing $function on $subdir oer samples ---\n";
    &execSamples ("sample_oer", $function);
    ...
    &execSamples ("sample_json", $function);
}
elsif ($nodir) {
    ...
    #Runs reader and writer
        ...
        if ($function eq "test") {
            ...
            if ($subdir eq "csharp") {
                ...
                elsif ( -e '../../lic_asn1rt.dll') {
                    `cp ../../lic_asn1rt.dll ./asn1rt.dll`;
                }
                else {
                    die ('C# run-time DLL file asn1rt.dll not found');
                }
            }

            printTestName($filename);
            execCmd($makeprog) or return;
            $msbuildvc6 = "../../../scripts/msbuildvc6.pl";
            if (($dirName =~ /^sample/) && (-e $msbuildvc6)) {
               $cmd = "..\\..\\..\\perlfromperl.bat $msbuildvc6 " . $filename;
               #execCmd($cmd) or return;
            }
...
sub doXerJavaMeTest {
   ...
   if (-e "EncDec.class") {
      $classpath = "../..";
      $classpath .= ";../../../javame_px/asn1rt.jar";
      $classpath .= ";../../xerces/xercesImpl.jar";
      $classpath .= ";../../xerces/xml-apis.jar";
