...
{
   TS24301IE_UESecurityCapability data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_UESecurityCapability (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "UESecurityCapability");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode UESecurityCapability failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_UESecurityCapability
         (&ctxt, "UESecurityCapability", &data);
   }
