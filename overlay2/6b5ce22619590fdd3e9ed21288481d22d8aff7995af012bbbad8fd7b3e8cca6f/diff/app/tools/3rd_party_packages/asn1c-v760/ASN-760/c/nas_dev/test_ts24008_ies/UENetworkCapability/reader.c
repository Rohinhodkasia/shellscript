...
{
   TS24301IE_UENetworkCapability data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   asn1Init_TS24301IE_UENetworkCapability (&data);

   ret = NASDec_TS24301IE_UENetworkCapability (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "UENetworkCapability");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode UENetworkCapability failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_UENetworkCapability
         (&ctxt, "UENetworkCapability", &data);
   }
