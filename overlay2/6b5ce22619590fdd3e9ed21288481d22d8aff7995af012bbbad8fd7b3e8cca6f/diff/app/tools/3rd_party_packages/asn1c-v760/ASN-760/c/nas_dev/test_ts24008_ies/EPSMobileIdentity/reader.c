...
{
   TS24301IE_EPSMobileIdentity data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_EPSMobileIdentity (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSMobileIdentity");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSMobileIdentity failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSMobileIdentity
         (&ctxt, "EPSMobileIdentity", &data);
   }
