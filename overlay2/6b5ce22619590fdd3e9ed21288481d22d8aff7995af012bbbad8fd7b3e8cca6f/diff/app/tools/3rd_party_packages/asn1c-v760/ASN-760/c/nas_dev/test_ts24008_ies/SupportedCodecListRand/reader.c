...
{
   TS24008IE_SupportedCodecList data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_SupportedCodecList (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "SupportedCodecList");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode SupportedCodecList failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_SupportedCodecList
         (&ctxt, "SupportedCodecList", &data);
   }
