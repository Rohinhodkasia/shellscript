...
{
   TS24301IE_APNAggMaxBitRateExt data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_APNAggMaxBitRateExt (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "APNAggMaxBitRateExt");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode APNAggMaxBitRateExt failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_APNAggMaxBitRateExt
         (&ctxt, "APNAggMaxBitRateExt", &data);
   }
