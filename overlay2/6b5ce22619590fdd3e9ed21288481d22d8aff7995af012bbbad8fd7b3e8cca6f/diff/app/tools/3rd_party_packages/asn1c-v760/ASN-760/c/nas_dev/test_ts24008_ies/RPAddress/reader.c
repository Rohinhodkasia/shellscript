...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24011IE_RPAddress (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "RPAddress");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode RPAddress failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24011IE_RPAddress
         (&ctxt, "RPAddress", &data);
   }
