...
{
   TS24301IE_NASSecParamsToEUTRA data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_NASSecParamsToEUTRA (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "NASSecParamsToEUTRA");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode NASSecParamsToEUTRA failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_NASSecParamsToEUTRA
         (&ctxt, "NASSecParamsToEUTRA", &data);
   }
