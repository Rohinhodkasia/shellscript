...
{
   TS24301IE_KSIAndSequenceNumber data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_KSIAndSequenceNumber (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "KSIAndSequenceNumber");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode KSIAndSequenceNumber failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_KSIAndSequenceNumber
         (&ctxt, "KSIAndSequenceNumber", &data);
   }
