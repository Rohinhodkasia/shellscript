...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24011IE_RPCauseElem (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "RPCauseElem");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode RPCauseElem failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24011IE_RPCauseElem
         (&ctxt, "RPCauseElem", &data);
   }
