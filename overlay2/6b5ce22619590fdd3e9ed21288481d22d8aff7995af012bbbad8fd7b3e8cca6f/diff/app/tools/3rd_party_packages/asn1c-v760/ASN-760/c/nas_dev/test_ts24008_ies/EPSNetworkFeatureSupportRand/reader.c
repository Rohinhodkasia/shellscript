...
{
   TS24301IE_EPSNetworkFeatureSupport data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_EPSNetworkFeatureSupport (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSNetworkFeatureSupport");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSNetworkFeatureSupport failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSNetworkFeatureSupport
         (&ctxt, "EPSNetworkFeatureSupport", &data);
   }
