...
{
   TS24301IE_EPSMobileIdentity_GUTI data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_EPSMobileIdentity_GUTI (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSMobileIdentity_GUTI");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSMobileIdentity_GUTI failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSMobileIdentity_GUTI
         (&ctxt, "EPSMobileIdentity_GUTI", &data);
   }
