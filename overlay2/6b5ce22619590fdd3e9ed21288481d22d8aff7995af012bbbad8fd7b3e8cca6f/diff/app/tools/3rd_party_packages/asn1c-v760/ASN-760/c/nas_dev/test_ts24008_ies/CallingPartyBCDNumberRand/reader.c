...
{
   TS24008IE_CallingPartyBCDNumber data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_CallingPartyBCDNumber (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "CallingPartyBCDNumber");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode CallingPartyBCDNumber failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_CallingPartyBCDNumber
         (&ctxt, "CallingPartyBCDNumber", &data);
   }
