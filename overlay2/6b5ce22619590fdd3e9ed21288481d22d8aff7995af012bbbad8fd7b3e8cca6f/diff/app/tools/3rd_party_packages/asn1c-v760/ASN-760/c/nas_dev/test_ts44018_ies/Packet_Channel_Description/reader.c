...
{
   TS44018IE_Packet_Channel_Description data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_Packet_Channel_Description (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Packet_Channel_Description");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Packet_Channel_Description failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_Packet_Channel_Description
         (&ctxt, "Packet_Channel_Description", &data);
   }
