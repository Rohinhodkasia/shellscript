...
{
   TS24008IE_ProtocolConfigOptions data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   asn1Init_TS24008IE_ProtocolConfigOptions (&data);

   ret = NASDec_TS24008IE_ProtocolConfigOptions (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "ProtocolConfigOptions");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode ProtocolConfigOptions failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_ProtocolConfigOptions
         (&ctxt, "ProtocolConfigOptions", &data);
   }
