...
{
   TS24008IE_EmergencyNumberList data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_EmergencyNumberList (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EmergencyNumberList");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EmergencyNumberList failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_EmergencyNumberList
         (&ctxt, "EmergencyNumberList", &data);
   }
