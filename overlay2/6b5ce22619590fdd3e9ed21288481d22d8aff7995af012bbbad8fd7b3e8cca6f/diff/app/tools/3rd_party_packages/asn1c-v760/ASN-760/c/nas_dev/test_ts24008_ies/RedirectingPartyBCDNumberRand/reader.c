...
{
   TS24008IE_RedirectingPartyBCDNumber data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_RedirectingPartyBCDNumber (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "RedirectingPartyBCDNumber");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode RedirectingPartyBCDNumber failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_RedirectingPartyBCDNumber
         (&ctxt, "RedirectingPartyBCDNumber", &data);
   }
