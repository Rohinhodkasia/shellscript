...
{
   TS44018IE_Cell_Sel_Ind_Post_Rel_TCH_SDCCH data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_Cell_Sel_Ind_Post_Rel_TCH_SDCCH (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Cell_Sel_Ind_Post_Rel_TCH_SDCCH");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Cell_Sel_Ind_Post_Rel_TCH_SDCCH failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_Cell_Sel_Ind_Post_Rel_TCH_SDCCH
         (&ctxt, "Cell_Sel_Ind_Post_Rel_TCH_SDCCH", &data);
   }
