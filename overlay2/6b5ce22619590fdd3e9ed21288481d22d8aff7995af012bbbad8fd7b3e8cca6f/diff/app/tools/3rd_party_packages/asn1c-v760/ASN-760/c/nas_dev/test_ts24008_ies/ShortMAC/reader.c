...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24301IE_ShortMAC (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "ShortMAC");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode ShortMAC failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_ShortMAC
         (&ctxt, "ShortMAC", &data);
   }
