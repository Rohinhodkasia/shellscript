...
{
   TS44018IE_Measurement_Results data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_Measurement_Results (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Measurement_Results");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Measurement_Results failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_Measurement_Results
         (&ctxt, "Measurement_Results", &data);
   }
