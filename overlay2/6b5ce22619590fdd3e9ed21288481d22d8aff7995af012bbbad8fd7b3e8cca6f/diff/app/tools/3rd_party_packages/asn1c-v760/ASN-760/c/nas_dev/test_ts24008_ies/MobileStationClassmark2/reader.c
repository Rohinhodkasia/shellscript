...
{
   TS24008IE_MobileStationClassmark2 data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_MobileStationClassmark2 (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "MobileStationClassmark2");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode MobileStationClassmark2 failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_MobileStationClassmark2
         (&ctxt, "MobileStationClassmark2", &data);
   }
