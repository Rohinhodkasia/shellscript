...
{
   TS44018IE_Restriction_Timer data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_Restriction_Timer (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Restriction_Timer");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Restriction_Timer failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_Restriction_Timer
         (&ctxt, "Restriction_Timer", &data);
   }
