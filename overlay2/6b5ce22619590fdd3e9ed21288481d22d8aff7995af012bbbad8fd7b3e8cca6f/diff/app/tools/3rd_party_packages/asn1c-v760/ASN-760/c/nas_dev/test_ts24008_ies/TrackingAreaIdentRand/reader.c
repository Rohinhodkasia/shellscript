...
{
   TS24301IE_TrackingAreaIdent data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_TrackingAreaIdent (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "TrackingAreaIdent");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode TrackingAreaIdent failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_TrackingAreaIdent
         (&ctxt, "TrackingAreaIdent", &data);
   }
