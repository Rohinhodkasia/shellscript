...
{
   TS24008IE_BearerCapability data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   asn1Init_TS24008IE_BearerCapability (&data);

   /* Call decode function */

   ret = NASDec_TS24008IE_BearerCapability (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "BearerCapability");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode BearerCapability failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_BearerCapability
         (&ctxt, "BearerCapability", &data);
   }
