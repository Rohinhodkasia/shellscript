...
{
   TS24301IE_NASSecurityAlgorithms data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_NASSecurityAlgorithms (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "NASSecurityAlgorithms");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode NASSecurityAlgorithms failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_NASSecurityAlgorithms
         (&ctxt, "NASSecurityAlgorithms", &data);
   }
