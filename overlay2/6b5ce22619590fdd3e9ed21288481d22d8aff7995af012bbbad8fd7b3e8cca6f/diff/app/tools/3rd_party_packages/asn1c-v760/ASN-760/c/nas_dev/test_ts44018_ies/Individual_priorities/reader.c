...
{
   TS44018IE_Individual_priorities data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_Individual_priorities (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Individual_priorities");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Individual_priorities failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_Individual_priorities
         (&ctxt, "Individual_priorities", &data);
   }
