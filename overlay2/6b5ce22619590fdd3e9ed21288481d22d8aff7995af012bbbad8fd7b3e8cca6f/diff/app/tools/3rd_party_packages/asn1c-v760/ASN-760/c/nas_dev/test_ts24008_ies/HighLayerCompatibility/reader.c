...
{
   TS24008IE_HighLayerCompatibility data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_HighLayerCompatibility (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "HighLayerCompatibility");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode HighLayerCompatibility failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_HighLayerCompatibility
         (&ctxt, "HighLayerCompatibility", &data);
   }
