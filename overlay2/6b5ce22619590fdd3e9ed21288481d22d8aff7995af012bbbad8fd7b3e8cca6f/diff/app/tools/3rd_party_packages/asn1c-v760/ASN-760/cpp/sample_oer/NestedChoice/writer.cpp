...
int main (int argc, char** argv)
{
   ...
   ASN1T_TestChoice data;
   ASN1C_TestChoice TestChoicePDU (encodeBuffer, data);
   

   data.t =  T_TestChoice_nest;
   data.u.nest = new_ASN1T_TestChoice_nest(TestChoicePDU);
   data.u.nest->t = T_TestChoice_nest_bw_color;
   data.u.nest->u.bw_color = TestChoice_nest_bw_color::color;

   if (trace) {
      printf ("The following record will be encoded:\n");
      asn1Print_TestChoice ("data", &data);
      printf ("\n");
   }

   /* Encode */

   if ((stat = TestChoicePDU.Encode ()) == 0)
   {
      if (trace) {
