...
{
   TS44018IE_RR_Packet_Downlink_Assignment_Type_2 data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_RR_Packet_Downlink_Assignment_Type_2 (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "RR_Packet_Downlink_Assignment_Type_2");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode RR_Packet_Downlink_Assignment_Type_2 failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_RR_Packet_Downlink_Assignment_Type_2
         (&ctxt, "RR_Packet_Downlink_Assignment_Type_2", &data);
   }
