...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24301IE_LCSIndicator (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "LCSIndicator");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode LCSIndicator failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_LCSIndicator
         (&ctxt, "LCSIndicator", &data);
   }
