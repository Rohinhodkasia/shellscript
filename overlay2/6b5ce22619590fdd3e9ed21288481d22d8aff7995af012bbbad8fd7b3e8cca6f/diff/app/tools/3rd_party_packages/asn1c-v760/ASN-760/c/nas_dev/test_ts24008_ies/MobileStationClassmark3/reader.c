...
{
   TS24008IE_MobileStationClassmark3 data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_MobileStationClassmark3 (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "MobileStationClassmark3");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode MobileStationClassmark3 failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_MobileStationClassmark3
         (&ctxt, "MobileStationClassmark3", &data);
   }
