...
{
   TS24008IE_ConnectedNumber data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_ConnectedNumber (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "ConnectedNumber");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode ConnectedNumber failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_ConnectedNumber
         (&ctxt, "ConnectedNumber", &data);
   }
