...
{
   TS44018IE_GPRS_broadcast_information data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_GPRS_broadcast_information (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "GPRS_broadcast_information");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode GPRS_broadcast_information failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_GPRS_broadcast_information
         (&ctxt, "GPRS_broadcast_information", &data);
   }
