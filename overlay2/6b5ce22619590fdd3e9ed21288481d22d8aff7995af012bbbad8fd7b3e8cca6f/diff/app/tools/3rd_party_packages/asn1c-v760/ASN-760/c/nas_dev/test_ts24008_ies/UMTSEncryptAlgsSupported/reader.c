...
{
   TS24301IE_UMTSEncryptAlgsSupported data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_UMTSEncryptAlgsSupported (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "UMTSEncryptAlgsSupported");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode UMTSEncryptAlgsSupported failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_UMTSEncryptAlgsSupported
         (&ctxt, "UMTSEncryptAlgsSupported", &data);
   }
