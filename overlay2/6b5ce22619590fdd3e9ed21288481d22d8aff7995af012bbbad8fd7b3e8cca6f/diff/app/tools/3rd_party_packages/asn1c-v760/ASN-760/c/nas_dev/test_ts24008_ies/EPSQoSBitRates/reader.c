...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24301IE_EPSQoSBitRates (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSQoSBitRates");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSQoSBitRates failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSQoSBitRates
         (&ctxt, "EPSQoSBitRates", &data);
   }
