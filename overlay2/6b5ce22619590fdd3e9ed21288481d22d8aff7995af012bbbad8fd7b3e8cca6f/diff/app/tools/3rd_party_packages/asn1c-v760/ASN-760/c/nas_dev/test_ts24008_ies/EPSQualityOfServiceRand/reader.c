...
{
   TS24301IE_EPSQualityOfService data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   asn1Init_TS24301IE_EPSQualityOfService (&data);

   /* Call decode function */

   ret = NASDec_TS24301IE_EPSQualityOfService (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSQualityOfService");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSQualityOfService failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSQualityOfService
         (&ctxt, "EPSQualityOfService", &data);
   }
