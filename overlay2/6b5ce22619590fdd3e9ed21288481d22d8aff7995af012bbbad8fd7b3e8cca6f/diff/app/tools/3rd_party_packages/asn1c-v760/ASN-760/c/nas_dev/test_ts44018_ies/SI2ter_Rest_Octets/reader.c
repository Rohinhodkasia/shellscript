...
{
   TS44018IE_SI2ter_Rest_Octets data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_SI2ter_Rest_Octets (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "SI2ter_Rest_Octets");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode SI2ter_Rest_Octets failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_SI2ter_Rest_Octets
         (&ctxt, "SI2ter_Rest_Octets", &data);
   }
