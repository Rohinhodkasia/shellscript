...
{
   TS24008IE_PacketDataProtoAddr data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_PacketDataProtoAddr (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "PacketDataProtoAddr");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode PacketDataProtoAddr failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_PacketDataProtoAddr
         (&ctxt, "PacketDataProtoAddr", &data);
   }
