...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS44018IE_BA_List_Pref (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "BA_List_Pref");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode BA_List_Pref failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_BA_List_Pref
         (&ctxt, "BA_List_Pref", &data);
   }
