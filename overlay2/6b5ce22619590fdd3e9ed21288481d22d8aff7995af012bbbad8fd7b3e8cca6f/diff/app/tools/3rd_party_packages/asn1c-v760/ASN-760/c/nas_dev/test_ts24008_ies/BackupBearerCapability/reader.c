...
{
   TS24008IE_BackupBearerCapability data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_BackupBearerCapability (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "BackupBearerCapability");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode BackupBearerCapability failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_BackupBearerCapability
         (&ctxt, "BackupBearerCapability", &data);
   }
