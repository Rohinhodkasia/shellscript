...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24301IE_Nonce (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Nonce");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Nonce failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
