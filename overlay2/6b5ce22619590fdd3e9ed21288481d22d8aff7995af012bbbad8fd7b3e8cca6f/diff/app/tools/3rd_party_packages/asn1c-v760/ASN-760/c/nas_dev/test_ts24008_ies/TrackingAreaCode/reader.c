...
{
   TS24301IE_TrackingAreaCode data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_TrackingAreaCode (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "TrackingAreaCode");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode TrackingAreaCode failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_TrackingAreaCode
         (&ctxt, "TrackingAreaCode", &data);
   }
