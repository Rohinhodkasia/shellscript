...
{
   TS24301IE_LCSClientIdentity data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_LCSClientIdentity (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "LCSClientIdentity");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode LCSClientIdentity failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_LCSClientIdentity
         (&ctxt, "LCSClientIdentity", &data);
   }
