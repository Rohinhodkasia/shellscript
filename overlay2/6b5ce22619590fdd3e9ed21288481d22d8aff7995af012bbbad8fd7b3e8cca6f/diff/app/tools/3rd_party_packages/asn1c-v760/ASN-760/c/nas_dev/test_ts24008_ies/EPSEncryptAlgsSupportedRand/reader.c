...
{
   TS24301IE_EPSEncryptAlgsSupported data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_EPSEncryptAlgsSupported (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSEncryptAlgsSupported");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSEncryptAlgsSupported failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSEncryptAlgsSupported
         (&ctxt, "EPSEncryptAlgsSupported", &data);
   }
