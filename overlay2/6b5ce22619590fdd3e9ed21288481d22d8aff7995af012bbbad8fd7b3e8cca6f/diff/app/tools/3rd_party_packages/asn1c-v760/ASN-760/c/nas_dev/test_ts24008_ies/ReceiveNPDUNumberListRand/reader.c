...
{
   TS24008IE_ReceiveNPDUNumberList data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_ReceiveNPDUNumberList (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "ReceiveNPDUNumberList");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode ReceiveNPDUNumberList failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_ReceiveNPDUNumberList
         (&ctxt, "ReceiveNPDUNumberList", &data);
   }
