...
int main (int argc, char** argv)
{
   ...
   ASN1TSeqOfList names;
   
   decodeBuffer.SetTrace (trace);

   stat = employeePDU.PPD_ChildName (&names);
   ...
   if (stat == 0) {
      if (trace) {
         ...
         printf ("\nIterate givenName elements...\n");
         ASN1CSeqOfList llist (decodeBuffer, names);
         ASN1CSeqOfListIterator* iter = llist.iterator ();
         int i = 0;
         char buf[40];
         while (iter->hasNext ()) {
            ASN1T_Name* pName;
            pName = (ASN1T_Name*)iter->next();
            os_snprintf (buf, sizeof(buf), "name[%d]", i++);
            asn1Print_Name (buf, pName);
         }
      }
   }
   else {
