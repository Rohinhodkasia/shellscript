...
{
   TS24301IE_EPSBearerContextStatus data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_EPSBearerContextStatus (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EPSBearerContextStatus");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EPSBearerContextStatus failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EPSBearerContextStatus
         (&ctxt, "EPSBearerContextStatus", &data);
   }
