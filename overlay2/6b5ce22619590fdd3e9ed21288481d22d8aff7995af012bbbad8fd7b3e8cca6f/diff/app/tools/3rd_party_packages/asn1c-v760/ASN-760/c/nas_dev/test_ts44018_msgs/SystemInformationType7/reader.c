...
int DecSI7 (OSCTXT* pctxt, const OSOCTET* msgbuf, OSSIZE len, OSBOOL verbose)
{
   TS44018IE_SI7_Rest_Octets data;
   int          ret;
   ...
   ret = NASDec_TS44018IE_SI7_Rest_Octets (pctxt, &data, TRUE );
   ...
   if (0 != ret) {
      printf ("decode SI 7 Rest Octets failed; status = %d\n", ret);
      rtxErrPrint (pctxt);
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_SI7_Rest_Octets(pctxt, "SI7 Rest Octets", &data);
   }
...
int main (int argc, char** argv)
{
   ...
   DecSI7 (&ctxt, data.restOctets.u.obj_SystemInformationType7->data,
            data.restOctets.u.obj_SystemInformationType7->numocts, verbose);
