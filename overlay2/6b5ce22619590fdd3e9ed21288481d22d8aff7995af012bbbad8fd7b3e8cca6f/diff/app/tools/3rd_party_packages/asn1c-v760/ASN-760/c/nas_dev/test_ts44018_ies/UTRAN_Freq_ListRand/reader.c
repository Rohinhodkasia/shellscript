...
{
   TS44018IE_UTRAN_Freq_List data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_UTRAN_Freq_List (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "UTRAN_Freq_List");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode UTRAN_Freq_List failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_UTRAN_Freq_List
         (&ctxt, "UTRAN_Freq_List", &data);
   }
