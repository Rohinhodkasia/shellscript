...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24301IE_ESMCause (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "ESMCause");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode ESMCause failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_ESMCause
         (&ctxt, "ESMCause", &data);
   }
