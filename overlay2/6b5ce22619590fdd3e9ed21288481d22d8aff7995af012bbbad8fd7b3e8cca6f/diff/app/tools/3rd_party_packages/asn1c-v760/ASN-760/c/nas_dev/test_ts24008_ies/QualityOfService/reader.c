...
{
   TS24008IE_QualityOfService data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_QualityOfService (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "QualityOfService");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode QualityOfService failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_QualityOfService
         (&ctxt, "QualityOfService", &data);
   }
