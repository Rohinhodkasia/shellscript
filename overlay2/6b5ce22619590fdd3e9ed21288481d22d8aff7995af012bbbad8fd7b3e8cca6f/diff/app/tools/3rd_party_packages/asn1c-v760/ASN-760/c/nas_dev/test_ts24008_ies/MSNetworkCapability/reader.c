...
{
   TS24008IE_MSNetworkCapability data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   asn1Init_TS24008IE_MSNetworkCapability (&data);

   /* Call decode function */

   ret = NASDec_TS24008IE_MSNetworkCapability (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "MSNetworkCapability");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode MSNetworkCapability failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_MSNetworkCapability
         (&ctxt, "MSNetworkCapability", &data);
   }
