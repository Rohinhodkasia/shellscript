...
{
   TS24301IE_TrackingAreaIdentListItem data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_TrackingAreaIdentListItem (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "TrackingAreaIdentListItem");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode TrackingAreaIdentListItem failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_TrackingAreaIdentListItem
         (&ctxt, "TrackingAreaIdentListItem", &data);
   }
