...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS24301IE_EMMCause (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "EMMCause");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode EMMCause failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_EMMCause
         (&ctxt, "EMMCause", &data);
   }
