...
{
   TS44018IE_RR_Packet_Uplink_Assignment data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_RR_Packet_Uplink_Assignment (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "RR_Packet_Uplink_Assignment");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode RR_Packet_Uplink_Assignment failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_RR_Packet_Uplink_Assignment
         (&ctxt, "RR_Packet_Uplink_Assignment", &data);
   }
