...
{
   TS44018IE_DTM_Information_Details data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_DTM_Information_Details (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "DTM_Information_Details");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode DTM_Information_Details failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_DTM_Information_Details
         (&ctxt, "DTM_Information_Details", &data);
   }
