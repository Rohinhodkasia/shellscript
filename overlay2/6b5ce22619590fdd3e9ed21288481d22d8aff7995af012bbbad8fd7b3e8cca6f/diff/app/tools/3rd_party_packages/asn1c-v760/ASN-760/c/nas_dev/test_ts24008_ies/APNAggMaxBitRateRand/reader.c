...
{
   TS24301IE_APNAggMaxBitRate data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_APNAggMaxBitRate (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "APNAggMaxBitRate");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode APNAggMaxBitRate failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_APNAggMaxBitRate
         (&ctxt, "APNAggMaxBitRate", &data);
   }
