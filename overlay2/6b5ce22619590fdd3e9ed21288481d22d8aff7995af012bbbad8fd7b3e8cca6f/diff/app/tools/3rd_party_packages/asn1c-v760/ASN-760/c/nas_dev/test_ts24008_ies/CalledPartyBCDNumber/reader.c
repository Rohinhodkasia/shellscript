...
{
   TS24008IE_CalledPartyBCDNumber data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_CalledPartyBCDNumber (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "CalledPartyBCDNumber");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode CalledPartyBCDNumber failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_CalledPartyBCDNumber
         (&ctxt, "CalledPartyBCDNumber", &data);
   }
