...
int main (int argc, char** argv)
{
   ...
   OSCrypt128Key integKey = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
                              0x98, 0x76, 0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba };
   OSCrypt128Key encryptKey = {  0x98, 0x76, 0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba,
   ...
   ret = rtx3gppAssignAlgorithmKeys(&ctxt, &integKey, &encryptKey, OS3GPPSecAlgorithm_AES, OS3GPPSecAlgorithm_AES);
   if ( ret == RTERR_NOTSUPP ) {
      printf ("Warning: unable to set AES algorithm; falling back to NULL algorithm.\n");
      rtxErrReset(&ctxt);
      ret = rtx3gppAssignAlgorithmKeys(&ctxt, NULL, NULL, OS3GPPSecAlgorithm_NULL_ALG, OS3GPPSecAlgorithm_NULL_ALG);
   }
   if (0 != ret) {
      rtxErrPrint (&ctxt);
   }
   ...
   pdu.data.t = T_TS24301Msgs_TS24301_IE_OBJECTSET_obj_ActvDedEPSBearerCtxtAcc;
   pdu.data.u.obj_ActvDedEPSBearerCtxtAcc = pvalue;

   asn1Init_TS24301Msg_ActvDedEPSBearerCtxtAcc (pvalue);
   pvalue->m.protocolConfigOptionsPresent = 1;
   pvalue->protocolConfigOptions.length = 47;
