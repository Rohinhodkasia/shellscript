...
{
   TS24301IE_NASSecParamsFromEUTRA data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_NASSecParamsFromEUTRA (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "NASSecParamsFromEUTRA");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode NASSecParamsFromEUTRA failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_NASSecParamsFromEUTRA
         (&ctxt, "NASSecParamsFromEUTRA", &data);
   }
