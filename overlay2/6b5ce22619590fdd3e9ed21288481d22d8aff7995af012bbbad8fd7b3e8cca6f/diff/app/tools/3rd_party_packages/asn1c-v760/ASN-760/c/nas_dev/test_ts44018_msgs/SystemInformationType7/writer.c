...
/**
 * Encode given SI 7 Rest Octets into given buffer.
 * @param pvalue
 ...
 * @return error code
 */
int EncodeSI7 (TS44018IE_SI7_Rest_Octets* pvalue, OSOCTET* msgbuf, OSSIZE* plen, OSBOOL verbose)
{
   OSCTXT ctxt;
   int ret;
   ...
   ret = NASEnc_TS44018IE_SI7_Rest_Octets (&ctxt, pvalue);
   if (0 != ret) {
      printf ("encode SI7 Rest Octets failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   *plen = rtxCtxtGetMsgLen (&ctxt);
   ...
   if (verbose) {
      printf ("Encoding of SI 7 Rest Octets was successful\n");
...
int main (int argc, char** argv)
{
   ...
   TS44018IE_SI7_Rest_Octets* pSI7;
   TS44018IE_SI7_Rest_Octets_Data si7data;
   OSCTXT	ctxt;
   OSCTXT*  pctxt = &ctxt;
   OSOCTET	msgbuf[MAXMSGLEN], *msgptr;
   OSOCTET  si7msgbuf[MAXMSGLEN];
   OSSIZE   si7len;
   ...
   /* Populate an SI 7 Rest Octets IE, encode it to a buffer, and set the
      encoding into an TS44018IE_SI7_Rest_Octets_Data object.
   */
   pSI7 = asn1Test_TS44018IE_SI7_Rest_Octets(pctxt);
   si7len = sizeof(si7msgbuf);
   ret = EncodeSI7 (pSI7, si7msgbuf, &si7len, verbose);
   if (0 != ret) return ret;  /* error already will have been printed */

   asn1Init_TS44018IE_SI7_Rest_Octets_Data (&si7data);
   si7data.numocts = si7len;
   si7data.data = si7msgbuf;
   ...
   pdu.msgType = ASN1V_ts44018Msg_mt_SystemInformationType7;
   pdu.data.t = T_TS44018Msgs_L2_PSEUDO_LEN_PDU_Messages_obj_SystemInformationType7;

   pdu.restOctets.t = T_TS44018Msgs_L2_PSEUDO_LEN_PDU_Messages_obj_SystemInformationType7;
   pdu.restOctets.u.obj_SystemInformationType7 = &si7data;
