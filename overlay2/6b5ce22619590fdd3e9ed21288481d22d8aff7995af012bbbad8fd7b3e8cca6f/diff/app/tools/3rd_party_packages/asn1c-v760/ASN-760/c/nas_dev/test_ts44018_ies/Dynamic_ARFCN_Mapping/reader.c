...
{
   TS44018IE_Dynamic_ARFCN_Mapping data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS44018IE_Dynamic_ARFCN_Mapping (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "Dynamic_ARFCN_Mapping");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode Dynamic_ARFCN_Mapping failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_Dynamic_ARFCN_Mapping
         (&ctxt, "Dynamic_ARFCN_Mapping", &data);
   }
