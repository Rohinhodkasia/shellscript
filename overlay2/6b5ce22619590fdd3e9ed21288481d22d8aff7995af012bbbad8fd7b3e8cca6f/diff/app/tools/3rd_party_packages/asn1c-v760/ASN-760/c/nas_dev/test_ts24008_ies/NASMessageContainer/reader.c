...
{
   TS24301IE_NASMessageContainer data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_NASMessageContainer (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "NASMessageContainer");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode NASMessageContainer failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_NASMessageContainer
         (&ctxt, "NASMessageContainer", &data);
   }
