...
{
   TS24008IE_CallControlCapabilities data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24008IE_CallControlCapabilities (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "CallControlCapabilities");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode CallControlCapabilities failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24008IE_CallControlCapabilities
         (&ctxt, "CallControlCapabilities", &data);
   }
