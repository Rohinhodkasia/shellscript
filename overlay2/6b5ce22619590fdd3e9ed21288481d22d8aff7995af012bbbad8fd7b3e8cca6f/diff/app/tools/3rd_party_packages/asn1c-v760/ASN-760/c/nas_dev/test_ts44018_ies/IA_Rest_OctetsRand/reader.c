...
int main (int argc, char** argv)
{
   ...
   ret = NASDec_TS44018IE_IA_Rest_Octets (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "IA_Rest_Octets");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode IA_Rest_Octets failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS44018IE_IA_Rest_Octets
         (&ctxt, "IA_Rest_Octets", &data);
   }
