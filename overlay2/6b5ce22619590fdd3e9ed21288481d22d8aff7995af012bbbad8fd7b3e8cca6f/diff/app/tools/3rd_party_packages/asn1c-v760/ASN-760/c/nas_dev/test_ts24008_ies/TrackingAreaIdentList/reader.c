...
{
   TS24301IE_TrackingAreaIdentList data;
   OSCTXT	ctxt;
   OSOCTET*     msgbuf;
   ...
   ret = NASDec_TS24301IE_TrackingAreaIdentList (&ctxt, &data);

   if (verbose) {
      printf ("Bit fields:\n");
      rtxDiagBitTracePrint (ctxt.pBitFldList, "TrackingAreaIdentList");
      printf ("\n");
   }

   if (0 != ret) {
      printf ("decode TrackingAreaIdentList failed; status = %d\n", ret);
      rtxErrPrint (&ctxt);
      return ret;
   }
   ...
   if (verbose) {
      ...
      asn1PrtToStrm_TS24301IE_TrackingAreaIdentList
         (&ctxt, "TrackingAreaIdentList", &data);
   }
