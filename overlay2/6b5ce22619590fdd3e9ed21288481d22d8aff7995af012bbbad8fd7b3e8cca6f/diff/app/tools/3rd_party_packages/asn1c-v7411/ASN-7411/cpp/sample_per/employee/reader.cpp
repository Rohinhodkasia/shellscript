...
int main (int argc, char** argv)
{
   ...
   if (stat == 0) {
      if (trace) {
         ...
         printf ("\nIterate ChildInformation elements...\n");
         ASN1T_ChildInformation* pChildInfo;
         ASN1CSeqOfList llist (decodeBuffer, employee.children);
         ASN1CSeqOfListIterator* iter = llist.iterator ();
         int i = 0;
         char buf[40];
         while (iter->hasNext ()) {
            pChildInfo = (ASN1T_ChildInformation*)iter->next();
            os_snprintf (buf, sizeof(buf), "child[%d]", i++);
            asn1Print_ChildInformation (buf, pChildInfo);
         }
      }
   }
   else {
